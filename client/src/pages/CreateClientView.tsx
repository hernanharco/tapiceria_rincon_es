...
export const CreateClientView = () => {
  ...
  const { clients, addClients, deleteClients, updateClients, refetchClients } =
    useClients();
  ...
  const handleSaveClient = async (cliente) => {
    try {
      ...
      toast.success(
        cliente.isEditing ? 'Cliente actualizado' : 'Cliente creado',
      );
    } catch (err) {
    ...
    if (step3.isConfirmed) {
      toast.promise(
        ...
        },
        {
          ...
          error:
            'Error: No se pudo eliminar el cliente (posibles datos vinculados).',
          duration: 5000,
        },
      );
    }
        ...
        <div className="flex flex-col lg:flex-row justify-between items-center gap-6 mb-12">
          <div className="text-center lg:text-left">
            ...
            <p className="text-slate-400 text-sm font-medium">
              Gestión integral de contactos y facturación.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-1 gap-4">
            {filteredClients.length > 0 ? (
              filteredClients.map((client, idx) => (
                <div
                  key={idx}
                  className="bg-white rounded-[2rem] md:rounded-none md:first:rounded-t-none md:last:rounded-b-[2rem] p-6 md:px-8 md:py-5 border border-slate-50 shadow-[0_10px_30px_rgba(0,0,0,0.02)] md:shadow-none md:grid md:grid-cols-5 md:items-center hover:bg-blue-50/30 transition-all"
                >
                  ...
                  <div className="flex items-center gap-3 mb-4 md:mb-0">
                    ...
                    <span className="text-slate-800 font-extrabold text-sm md:font-bold">
                      {client.name}
                    </span>
                  </div>
                  ...
                  <div className="flex justify-between md:justify-center items-center pt-4 md:pt-0 border-t md:border-none border-slate-50">
                    <span className="md:hidden text-[10px] font-black text-slate-300 uppercase">
                      Acciones
                    </span>

...
export const FootersProvider = ({ children }) => {
  ...
  const cargarFooters = useCallback(
    async (silent = false) => {
  ...
  // 2. Guardar nuevo footer (POST)
  const saveFooter = async (nuevoFooter) => {
    console.log("Iniciando saveFooter en API...", nuevoFooter);

    try {
      // Forzamos que los valores sean numéricos antes de enviar
      const dataToPost = {
        subtotal: parseFloat(nuevoFooter.subtotal) || 0,
        base_imponible: parseFloat(nuevoFooter.base_imponible) || 0,
        iva: parseFloat(nuevoFooter.iva) || 0,
        total: parseFloat(nuevoFooter.total) || 0,
        footer_documento: nuevoFooter.footer_documento, // El ID de la cabecera
      };

      const res = await api.post(API_URL, dataToPost);

      // Actualizamos el estado local para que la UI se entere al instante
      setFooters((prev) => [...prev, res.data]);

      return res.data;
    } catch (err) {
      console.error("Error al guardar el footer en la base de datos:", err);
      throw new Error(
        err.response?.data?.detail || "Error al crear los totales"
      );
    }
  };

  // 3. Búsqueda LOCAL (Cero latencia con Frankfurt)
  const fetchFooterByNum = useCallback(
    (documentoId) => {
  ...
  const getFootersByFieldId = useCallback(
    (id) => {

...
export const Sidebar = () => {
  ...
  const { empresas, loading } = useCompany(); // Obtenemos los datos de la empresa
  ...
  // Obtenemos la primera empresa (generalmente habrá solo una)
  const company = empresas && empresas.length > 0 ? empresas[0] : null;
      ...
      <div className="flex h-screen w-full bg-[#F8FAFC]">
        ...
        >
          ...
          <div className="relative flex items-center h-24 px-6 mb-4">
            <div
              className={`flex items-center gap-3 overflow-hidden transition-all duration-500 ${isOpen ? 'opacity-100' : 'opacity-0 invisible'}`}
            >
              {company?.logo ? (
                <img
                  src={company.logo}
          ...
          <nav className="flex-1 px-4 space-y-2 overflow-y-auto custom-scrollbar">
            <ul className="space-y-2">
              {MENU_ITEMS.map(
                ({ label, path, icon: IconComponent, activeColor }) => {
                  ...
                  return (
                    <li key={path}>
                      ...
                      >
                        ...
                        {isOpen && (
                          <span
                            className={`text-sm font-bold tracking-tight transition-all duration-300 ${isActive ? 'opacity-100' : 'opacity-70 group-hover:opacity-100'}`}
                          >
                            {label}
                          </span>
                        )}
          ...
          <div className="p-6">
            <div
              className={`p-4 rounded-2xl bg-slate-800/50 border border-slate-700/50 transition-all duration-500 ${!isOpen && 'items-center justify-center flex'}`}
            >
              {isOpen ? (

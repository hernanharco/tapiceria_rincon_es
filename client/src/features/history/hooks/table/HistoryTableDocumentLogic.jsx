...
import { HistoryTableDocumentView } from '@/features/history/components/table/HistoryTableDocumentView';
import { HistoryModals } from '@/features/clients/components/modals/DocumentCreateModal';
import DateModal from '@/utils/dateModal';
import useHistoryTableDocument from '@/features/history/hooks/useHistoryTableDocument';
...
export const HistoryTableDocumentLogic = ({
  ...
  const handleFilter = (startDate, endDate, documentType) => {
    ...
    const finalItems = sortedProducts.filter((item) => {
      ...
      const fFactura = item.fecha_factura ? dayjs(item.fecha_factura) : null;
      const fAlbaran = item.fecha_factalb ? dayjs(item.fecha_factalb) : null;
      const fDateFact = item.datefactura ? dayjs(item.datefactura) : null;

      const cumpleFecha =
        !fechaInicio ||
        !fechaFin ||
        [fFactura, fAlbaran, fDateFact].some((f) => {
          if (!f) return false;
          return (
            (f.isAfter(fechaInicio) || f.isSame(fechaInicio)) &&
            (f.isBefore(fechaFin) || f.isSame(fechaFin))
          );
        });
      ...
      if (documentType !== 'Todos') {
        if (documentType === 'Presupuesto') {
          cumpleTipo = item.num_presupuesto?.toUpperCase().startsWith('PRE');
        } else if (documentType === 'Albarán') {
          cumpleTipo = item.num_albaran?.toUpperCase().startsWith('ALB');
        } else if (documentType === 'Factura') {
          cumpleTipo = item.num_factura?.toUpperCase().startsWith('FAC');
        }
      }
  ...
  const handleClearFilter = () => {
    // 1. Desactivamos la bandera de filtrado
    setIsFiltering(false);

    // 2. Restauramos los resultados a la lista completa actual
    setFilteredResults(sortedProducts);

    // Opcional: Feedback visual
    toast.info('Filtros de fecha y tipo limpiados');
  };

  return (
    <>
      <HistoryTableDocumentView
        sortedProducts={filteredResults}
